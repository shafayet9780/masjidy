...
Deno.serve(async (req) => {
  ...
  if (rowErr || !row) {
    return jsonResponse({ error: 'NOT_FOUND', message: 'Confirmation not found' }, 404);
  }
  ...
  const { data: prayerRows, error: prErr } = await admin
    .from('jamat_times')
    .select('prayer')
    .eq('mosque_id', mosqueId)
    .in('status', ['live', 'pending']);
  ...
  if (prErr) {
    return jsonResponse({ error: 'SERVER_ERROR', message: 'Could not load jamat times' }, 500);
  }

  const prayers = [...new Set((prayerRows ?? []).map((r) => String(r.prayer)))];
